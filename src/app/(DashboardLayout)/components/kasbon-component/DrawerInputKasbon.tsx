...
interface DrawerInputKasbonProps {
  open: boolean;
  onClose: () => void;
  onSuccess: () => void;
}
...
const DrawerInputKasbon: React.FC<DrawerInputKasbonProps> = ({ open, onClose, onSuccess }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch('http://127.0.0.1:8000/api/kasbon', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        setSnackbarMessage('Kasbon berhasil ditambahkan!');
        ...
        setSnackbarMessage('An error occurred while adding the kasbon');
        setSnackbarSeverity('error');
    } finally {
      setLoading(false);
    }
      ...
      <Drawer anchor="right" open={open} onClose={onClose}>
        <Box sx={{ width: 400, padding: 3 }}>
          <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
            <Typography variant="h6">Tambah Kasbon</Typography>
...
export default DrawerInputKasbon;
