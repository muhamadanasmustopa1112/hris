...
const PerizinanEdit: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchPerizinan = async () => {
      if (id) {
        try {
          const response = await axios.get(`https://hris-api.ptspsi.co.id/api/perizinan/${id}`, {
            headers: {
              'Authorization': `Basic ${basicAuth}`
            }
          });
          ...
          const employeeExists = employees.some(emp => emp.id === data.companies_user_id);

          setFormData({
            _method: 'PUT',
            jenis_izin_id: data.jenis_perizinan_id || '',
            category_id: data.category_id || '',
            companies_user_id:  employeeExists ? data.companies_user_id : '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    e.preventDefault();
    try {
      const response = await axios.post(`https://hris-api.ptspsi.co.id/api/perizinan/${id}`, formData, {
        headers: {
      ...
      setAlertMessage('Perizinan berhasil diupdate!');
