...
const TablePresensi: React.FC<TablePresensiProps> = ({
  ...
  const deletePresensi = async (id_masuk: any, id_keluar: any) => {
    ...
    if (confirmDelete) {
      try {
        const response = await fetch(`http://127.0.0.1:8000/api/presensi-masuk/${id_masuk}`, {
          method: 'DELETE',
          headers: {
        ...
        const response2 = await fetch(`http://127.0.0.1:8000/api/presensi-keluar/${id_keluar}`, {
          method: 'DELETE',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${Cookies.get('token')}`,
